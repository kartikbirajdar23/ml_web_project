...
# ============================================================
    ...
    st.write(
        "District-wise traffic congestion visualization "
        "for Maharashtra."
    )
    ...
    maharashtra_df = pd.read_csv(maharashtra_file)
    ...
    for _, row in maharashtra_df.iterrows():
        ...
        popup_text = f"""
        <b>{row['District']}</b><br>
        🚗 Vehicles: {row['Vehicles']}<br>
        🛣️ Road Capacity: {row['Road_Capacity']}<br>
        🚨 Accidents: {row['Accidents']}<br>
        📊 Congestion: {row['Congestion_Percent']}%<br>
        🚦 Traffic Level: {row['Traffic_Level']}
        """
    ...
    with c2:
        st.metric(
            "🚗 Total Vehicles",
            f"{maharashtra_df['Vehicles'].sum():,}"
        )

    with c3:
        st.metric(
            "🚨 Total Accidents",
            int(maharashtra_df["Accidents"].sum())
        )
